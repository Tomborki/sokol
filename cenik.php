<!DOCTYPE html>
<html>
    <head>

    <?php require_once("meta.php"); ?>

    <link href="css/12cols.css" rel="stylesheet">
    <link href="css/styl.css" rel="stylesheet">
    <link href="css/media_1000px.css" rel="stylesheet">
    <link href="css/media_520px.css" rel="stylesheet">
    <link href="css/styl_cenik.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Signika:400,700" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Hammersmith+One" rel="stylesheet">



    </head>
    <?php require_once("header.php"); ?>

          <!----------------------------------------------------------------------------------------------------------------------->









          <h2 id="progres_nadpis"> Ceník - pronájem vnitřních sportovišť </h2>
          <table class="tabulka_cenik">
            <tr>
              <th></th>
              <th>Cena pro nečleny</th>
              <th>Cena pro členy</th>
            </tr>
            <tr>
              <td>Tělocvična</td>
              <td>200 Kč/hod.<br>150 Kč/hod.*</td>
              <td>100 Kč/hod.<br><span>mimo rozvrh</span><br>100 Kč/hod. <br><span>činnost dle rozvrhu</span></td>
            </tr>
            <tr>
              <td>Malá tělocvična</td>
              <td>120 Kč/hod.<br>100 Kč/hod. *</td>
              <td>60 Kč/hod.<br>50 Kč/hod. <br><span>(při dlouhodobém využívání)</span></td>
            </tr>
            <tr>
              <td>Přísálí, jeviště, tělocvična pro speciální účely (např. bez sprch)</td>
              <td>100 Kč/hod.<br>75 Kč/hod. <br><span>(dlouhodobý pronájem)</span></td>
              <td>50 Kč/hod.</td>
            </tr>
            <tr>
              <td>Místnost pro stolní tenis</td>
              <td>60 Kč/hod. <br><span>jednorázový vstup i dlouhodobý pronájem</span></td>
              <td>40 Kč/hod.</td>
            </tr>
            <tr>
              <td>Zasedací místnost</td>
              <td>50 Kč/hod. <span>- všední den</span><br>70 Kč/hod. <span>- víkendy</span></td>
              <td>zdarma <br><span>(pro členy výboru, ostatní jako nečlenové)</span></td>
            </tr>
          </table>
          <p>* - dlouhodobý pronájem, podmínkou je uzavření smlouvy na min. 2 měsíce a úhrada předem</p>
          <p>V ceně za pronájem je zahrnuto použití šaten, sociálních zařízení a sprch.</p>



          <h2 id="progres_nadpis"> Ceník - pronájem venkovních sportovišť </h2>
          <table class="tabulka_cenik">
            <tr>
              <th></th>
              <th>Všední dny 9 až 15 hodin<br><span>(pro členy i nečleny)</span></th>
              <th>Všední dny po 15. hodině<br><span>(pro členy i nečleny)</span></th>
            </tr>
            <tr>
              <td>Antukový kurt pro volejbal, nohejbal, tenis</td>
              <td>100 Kč/hod.<br><span>(jednorázově i dlouhodobě)</span></td>
              <td>140 Kč/hod.<br><span>(jednorázově i dlouhodobě)</span></td>
            </tr>
            <tr>
              <td>Beachvolejbalový kurt</td>
              <td>100 Kč/hod.<br><span>(jednorázově i dlouhodobě)</span></td>
              <td>140 Kč/hod.<br><span>(jednorázově i dlouhodobě)</span></td>
            </tr>
            <tr>
              <td>Travnatá plocha</td>
              <td>400 Kč/hod.</td>
              <td>400 Kč/hod.</td>
            </tr>
            <tr>
              <td>Hřiště na petangue</td>
              <td>50 Kč/hod.</td>
              <td>50 Kč/hod.</td>
            </tr>
          </table>
          <p>V ceně za pronájem kurtu je zahrnuto použití šaten, sociálních zařízení a sprch.</p>



          <h2 id="progres_nadpis"> Ceník za služby </h2>
          <table class="tabulka_cenik">
            <tr>
              <th style="text-align: left;">Popis služby</th>
              <th>Cena</th>
            </tr>
            <tr>
              <td>Vystavení průkazky ČOS při vzniku členství</td>
              <td>30 Kč</td>
            </tr>
            <tr>
              <td>Vystavení duplikátu při ztrátě průkazky</td>
              <td>30 Kč</td>
            </tr>
            <tr>
              <td>Vystavení duplikátu při zaplnění průkazky známkami</td>
              <td>zdarma</td>
            </tr>
          </table>

 <!---------------------------------------------------------------------------------------------------------------------->

          <?php require_once("footer.php"); ?>

</html>
